...
# Erstellt den ByteTrack-Tracker passend zur installierten supervision-Version
...
# Erstellt eine Polygon-Zone, in der später geprüft wird, ob Personen diese betreten
...
# Holt die Tracking-IDs aus den erkannten Objekten
...
# Speichert eine bereits vorbereitete Zone für die aktuelle Bildauflösung
@dataclass
...
# Speichert gemittelte Laufzeiten für Performance-Anzeige/Debugging
@dataclass
class FrameZeiten:
    quell_ms: float = 0.0
...
class AlarmSteuerung:
    ...
    # Löst einen Alarm aus, aber nur wenn der Cooldown abgelaufen ist
    ...
    # Text, der kurz nach einem Alarm im Bild angezeigt werden kann
...
class UeberwachungsMonitor:
    ...
    # Glättet Laufzeitwerte, damit sie nicht stark springen
    def _zeit_aktualisieren(self, name: str, wert_ms: float) -> None:
        bisheriger_wert = getattr(self._zeiten, name)
        neuer_wert = (
            wert_ms
            if bisheriger_wert == 0.0
            else bisheriger_wert * 0.8 + wert_ms * 0.2
        )
        setattr(self._zeiten, name, neuer_wert)

    # Setzt Tracker und Zonen-Cache zurück
    ...
    # Startet je nach Einstellung Snapshot- oder Stream-Modus
    ...
    # Liest dauerhaft den Videostream und verarbeitet immer das neueste Frame
    ...
    # Holt in festen Abständen einzelne Bilder vom ESP32
    ...
    # Zentrale Verarbeitung eines Frames: Erkennung, Tracking, Zonenprüfung, Zeichnen
    def frame_verarbeiten(self, frame: np.ndarray) -> np.ndarray:
        ...
        if self._letztes_frame_zeit:
            ...
            if delta > 0:
                ...
                self._fps = (
                    aktuelle_fps
                    if self._fps == 0.0
                    else self._fps * 0.9 + aktuelle_fps * 0.1
                )

        self._letztes_frame_zeit = jetzt

        # 1. Personen mit YOLO erkennen
        ...
        # 2. Erkannte Personen über mehrere Frames verfolgen
        ...
        # 3. Prüfen, ob getrackte Personen eine No-Go-Zone betreten
        ...
        # 4. Ergebnisbild mit Boxen, IDs und Zonen zeichnen
    ...
    # YOLO erkennt nur Personen, also Klasse 0 aus dem COCO-Datensatz
    ...
    # Skaliert die gespeicherten Zonen auf die aktuelle Bildauflösung
    ...
    # Zeichnet Zonen, Bounding-Boxes, Tracking-IDs und Sicherheitswerte ins Bild
...
# Aliasname, falls anderer Code diesen Namen erwartet
