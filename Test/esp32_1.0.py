...
class Esp32KameraClient:
    ...
    # Holt ein einzelnes JPEG-Bild vom ESP32 und wandelt es in ein OpenCV-Bild um
    ...
    # Öffnet den MJPEG-Stream des ESP32, bevorzugt mit FFmpeg-Backend
    ...
    # Schaltet den Buzzer am ESP32 für eine bestimmte Zeit ein
    ...
    # Schaltet den Buzzer am ESP32 aus
...
class NeuestesFrameStream:
    def __init__(self, kamera_client: Esp32KameraClient) -> None:
        ...
        # Sperre und Stop-Event für den Hintergrund-Thread
    ...
    # Startet einen Hintergrund-Thread, der dauerhaft Frames liest
    def start(self) -> None:
    ...
    # Liest kontinuierlich aus dem Stream und speichert nur das neueste Frame
    def _lauf(self) -> None:
        ...
        while not self._stop_ereignis.is_set():
            ...
            # Gleitender Durchschnitt der Lesezeit
            self.quell_lese_ms = (
                lese_ms
            ...
            # Nur das neueste Frame wird gespeichert, alte Frames werden verworfen
            with self._sperre:
                self._neuestes_frame = frame
                self._neueste_sequenz += 1

    # Gibt ein neues Frame zurück, aber nur wenn seit dem letzten Lesen eins angekommen ist
    def neuestes_lesen(
    ...
    # Stoppt den Hintergrund-Thread und gibt die Kamera-/Stream-Ressource frei
...
# Englische Aliasnamen, falls andere Dateien diese Klassennamen importieren
